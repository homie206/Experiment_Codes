...
def get_model_examing_result(model_id, iteration):
    for mbti_item in single_prompt_template["mbti_prompt"]:
        ...
        output_file_name = os.path.join(output_folder, f'{mbti_label_content}-16p-llama3.2-3b-instruct-output.txt')
...
def main_run(model_id):
    for itr in range(10):
        ...
        for item in test_template["combinations"]:
            ...
            output_file_name = f'our_method/result_iteration_{itr + 1}/result-generate-{label_content_str}-16p-llama3.2-3b-instruct-output.txt'
            result_file_name = f'our_method/result_iteration_{itr + 1}/result-generate-{label_content_str}-16p-llama3.2-3b-instruct-result.csv'
            ...
            with open(output_file_name, 'a', encoding='utf-8') as f, open(result_file_name, 'a', encoding='utf-8') as r:
                ...
                for question_num, question in mbti_questions.items():
                    ...
                    response = get_response(question=question, pip_line=pipeline, prompt= item["prompt"], prompt_by_words= prompt_by_words)
