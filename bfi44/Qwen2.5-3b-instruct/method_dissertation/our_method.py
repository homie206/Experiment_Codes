...
def get_response(question, tokenizer, prompt, prompt_by_words, model):
...
def get_model_examing_result(iteration):
    for mbti_item in single_prompt_template["mbti_prompt"]:
        ...
        output_file_name = os.path.join(output_folder, f'{mbti_label_content}-bfi44-Qwen2.5-3B-Instruct-output.txt')
...
def get_multi_model_examing_result(iteration):
...
def main_run():
    ...
    for itr in range(10):
        ...
        get_model_examing_result(itr + 1)
        get_multi_model_examing_result(itr + 1)
        ...
        for item in test_template["combinations"]:
            ...
            output_file_name = f'our_method/result_iteration_{itr + 1}/result-generate-{label_content_str}-bfi44-qwen2.5-3b-instruct-output.txt'
            result_file_name = f'our_method/result_iteration_{itr + 1}/result-generate-{label_content_str}-bfi44-qwen2.5-3b-instruct-result.csv'
            ...
            with open(output_file_name, 'a', encoding='utf-8') as f, open(result_file_name, 'a', encoding='utf-8') as r:
                with open('../bfi44.txt', 'r') as f2:
                    ...
                    for q in question_list:
                        ...
                        answer = get_response(question=q, tokenizer= tokenizer, prompt= item["prompt"], prompt_by_words= prompt_by_words, model=model_qw)
