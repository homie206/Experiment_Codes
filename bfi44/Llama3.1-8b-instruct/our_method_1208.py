...
def get_llm_response(question,pip_line,prompt):
    pipeline = pip_line
    messages = [
        {"role": "system", "content":prompt},
        {"role": "user", "content": "Imagine you are a human, briefly describe whether " + question}
    ]
...
def main_run(model_id):
    ...
    for itr in range(10):
        get_model_examing_result(model_id, itr + 1) #llm产生单个维度的词语
        get_multi_model_examing_result(model_id, itr + 1) #llm产生多个维度的词语
        ...
        for item in test_template["combinations"]:
            ...
            with open(output_file_name, 'a', encoding='utf-8') as f, open(result_file_name, 'a', encoding='utf-8') as r:
                ...
                for questions in questionnaires["questions"]:
                    ...
                    #question1 = "you are talkative."
                    #question2 = questions["q2"]

                    llm_gen_descriptions = get_llm_response(question= question1, pip_line=pipeline, prompt= item["prompt"])
                    print("Do you agree with that " + question1)
                    print(llm_gen_descriptions)

                    # 初始化模型
                    str1 = get_entailment_words(question = llm_gen_descriptions, string= adjective_string)
                    #str2 = get_entailment_words(question = question2, string=adjective_string)
                    str1_str1 = [word.strip().lower() for word in str1.split(",")]
                    df_str = pd.read_csv(new_output)
                    df_str_lower = df_str.applymap(lambda x: x.lower() if isinstance(x, str) else x)

                    str_results = []
                    #unique_results = pd.DataFrame(str_results).drop_duplicates().to_dict(orient='records')

                    for adjective in str1_str1:
                        # 找到包含该形容词的所有行和列
                        match_rows = df_str_lower.apply(lambda row: adjective in row.values, axis=1)
                        match_cols = df_str_lower.columns[df_str_lower.isin([adjective]).any()].tolist()

                        # 提取行标签
                        matched_row_labels = df.index[match_rows].tolist()

                        # 添加结果
                        # 假设 DataFrame 中有一列 'Label' 包含行对应的标签名称
                        for row in matched_row_labels:
                            row_label = df.at[row, 'Label']  # 从 'Label' 列中获取该行的标签名称
                            for col in match_cols:
                                str_results.append({"Adjective": adjective, "Label": row_label, "Column": col})

                    # 转为 DataFrame 输出结果
                    print("str_labels:" + str(str_results))
                    #print(f"str_labels: {str_results}")



                    #prompt_by_words = f"In the current situation, here are more adjectives that describe your personality: {str1}, {str2}. "
                    prompt_by_words = f"In the current situation, here are more adjectives that describe your personality: {str1}"
